...
system.time(
  ...
  max_cusum_matrix <- foreach::foreach(
    ...
    for (j in 1:num_of_k_values) {
      ...
      vec_time_excess_sim_out <- CUSUMrelsurv::vec_excess_sim_acc_time_lin_piecewise_out_traditional(
        partition_t_vec = sim_obs_run$partition_t_vec,
        baseline_vec = sim_obs_run$baseline_vec,
...
saveRDS(max_cusum_matrix, file = "section_3_4/sim_cusum_check_performance_eta_star.RDS")

# If the simulations has been performed

max_cusum_matrix <- readRDS("section_3_4/sim_cusum_check_performance_eta_star.RDS")
...
saveRDS(cusum_signal_df, "section_3_4/cusum_signal_df_eta_star.RDS")
