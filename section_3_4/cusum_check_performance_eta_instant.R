...
sim_obs_func <- function() {
  ...
  original_follow_up_interval_partition <- c(seq(0, 5, by = 1), 10) # originally 6 bands, yearly bands for the first 5 years and a larger band between 5 and 10 years of follow up
  
  # Now extend the last band which corresponds to the time period between 5 and 10 years of follow up to 5 and 20 years of follow up for positive coefficient of accelerated time equal to 2 or less. 
  # But still interest in 10 year survival, hence the original observation time will still be 10 years at largest. 
  
  follow_up_interval_partition <- original_follow_up_interval_partition
  follow_up_interval_partition[length(follow_up_interval_partition)] <- 20  
...
n_iterations <- 1000
k_vec <- c(0.9, 0.95, 1.05, 1.1) # test out for different values of k
num_of_k_values <- length(k_vec)
...
system.time(
  ...
  max_cusum_matrix <- foreach::foreach(
    ...
    store_max_cusum <- numeric(num_of_k_values)
    ...
    # Observations experience out of control right away (eta = 0) 
    
    for (j in 1:num_of_k_values) {
      
      u_excess_vec <- runif(sim_obs_run$n_obs)
      
      vec_time_excess_sim <- CUSUMrelsurv::vec_excess_sim_acc_time_lin_piecewise_out_traditional(
      ...
      cusum_chart <- CUSUMrelsurv::cusum_acc_time_lin_r_t_piecewise_traditional(
        partition_t_vec = sim_obs_run$partition_t_vec,
        baseline_vec = sim_obs_run$baseline_vec,
        start_year = sim_obs_run$start_year,
        age_vec = sim_obs_run$age_vec,
...
colnames(max_cusum_matrix) <- paste("k_", k_vec, sep = "")

saveRDS(max_cusum_matrix, file = "section_3_4/sim_cusum_check_performance_eta_instant.RDS")

# If the simulations has been performed

max_cusum_matrix <- readRDS("section_3_4/sim_cusum_check_performance_eta_instant.RDS")

max_cusum_matrix_threshold <- readRDS("section_3_4/sim_cusum_in_control_samples_for_threshold.RDS")
...
cusum_check_combinations <- expand.grid(false_prob = false_prob_vec, k = k_vec)
cusum_check_combinations_indices <- expand.grid(false_prob_index = 1:length(false_prob_vec), k_index = 1:length(k_vec))
...
cusum_signal_df <- data.frame(
  eta = rep("eta = 0", nrow(cusum_check_combinations)),
...
saveRDS(cusum_signal_df, "section_3_4/cusum_signal_df_eta_instant.RDS")
