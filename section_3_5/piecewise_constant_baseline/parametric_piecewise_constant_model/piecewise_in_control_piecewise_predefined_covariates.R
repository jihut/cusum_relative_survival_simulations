...
system.time(
  for (i in 1:n_sims) {
    ...
    sim_obs_func <- function(base_piecewise_excess_model) {
      ...
      beta_vec <- base_piecewise_excess_model$coefficients[1:ncol(x_matrix)] # simulate excess times based on the estimated coefficients of the piecewise excess hazard baseline model
      ...
      chi_vec <- base_piecewise_excess_model$coefficients[-(1:ncol(x_matrix))] # estimated baseline hazard based on the piecewise baseline excess hazard model
    ...
    max_cusum_matrix <- CUSUMrelsurv::sim_predefined_obs_func_cusum_prop_threshold_piecewise_function_parallel(
      ...
      base_piecewise_model = base_piecewise_excess_model,
      n_iterations = 1000,
      n_cores = 8, 
      rho_vec = c(0.9),
...
list_piecewise_in_control_piecewise_predefined <- list(threshold_values = store_threshold, proportion_in_control_signal = proportion_signals_in_control_estimated_vs_true)
saveRDS(list_piecewise_in_control_piecewise_predefined, "section_3_5/piecewise_constant_baseline/parametric_piecewise_constant_model/list_piecewise_in_control_piecewise_predefined_covariates.RDS")

# If the simulations have been performed

list_piecewise_in_control_piecewise_predefined <- readRDS("section_3_5/piecewise_constant_baseline/parametric_piecewise_constant_model/list_piecewise_in_control_piecewise_predefined_covariates.RDS")

df_piecewise_in_control_piecewise_predefined <- data.frame(proportion_in_control_signal = list_piecewise_in_control_piecewise_predefined$proportion_in_control_signal)

hist(df_piecewise_in_control_piecewise_predefined$proportion_in_control_signal, breaks = seq(from = 0, to = 0.45, by = 0.01), probability = T)

ggplot(df_piecewise_in_control_piecewise_predefined, aes(x = proportion_in_control_signal, y = after_stat(density))) + 
  ...
  labs(
    title = "Histogram of signal ratio under in-control (piecewise baseline excess hazard model and true covariate distributions)",
    x = "Signal ratio",
    y = "Density"
  ) + 
  theme(
