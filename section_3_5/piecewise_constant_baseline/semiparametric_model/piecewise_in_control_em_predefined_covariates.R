...
base_chi_vec <- c(-1.4, -1.6, -1.8, -2, -2.1, -3) 

base_baseline_excess_vec <- exp(base_chi_vec) # baseline excess hazard for a time point in a given band - chi above is defined to coincide with e.g. excess hazard models with piecewise constant baseline from relsurv
...
system.time(
  for (i in 1:n_sims) {
    ...
    base_vec_time_excess_sim <- CUSUMrelsurv::vec_excess_sim_piecewise(
      partition_t_vec = follow_up_interval_partition,
      baseline_vec = base_baseline_excess_vec,
      max_follow_up_vec = base_max_follow_up_vec,
      u_vec = base_u_excess_vec,
      linear_predictor_vec = base_x_matrix %*% base_beta_vec
    )
    ...
    base_delta_i_vec <- as.integer(pmax(base_delta_p_vec, as.numeric(base_vec_time_excess_sim < base_time_pop_sim)) * as.numeric(base_time_censoring_sim > base_time_observed_sim))
    ...
    # Fit a excess model using EM based on this dataset 
    
    base_em_excess_model <-
      rsadd(
    ...
    base_estimated_beta_vec <- base_em_excess_model$coefficients
    
    # Check cumulative baseline excess and baseline excess
    
    # Baseline approximate 
    
    base_data_baseline_fit <- as.data.frame(cbind(base_em_excess_model$lambda0 * 365.241, base_em_excess_model$times / 365.241)) # everything in year scale 
    colnames(base_data_baseline_fit) <- c("baseline_haz_values", "time_in_years")
    
    base_baseline_haz_fit <- smooth.spline(base_data_baseline_fit$time_in_years, base_data_baseline_fit$baseline_haz_values, df = 25) # also need the baseline hazard at any time point other than the times to event from data
    
    base_baseline_haz_vec_general_func <- function(t){ # t in years 
      baseline_value <- predict(base_baseline_haz_fit, t)$y
    ...
    base_data_cumulative_baseline_fit <- as.data.frame(cbind(base_em_excess_model$Lambda0, base_em_excess_model$times / 365.241)) # everything in year scale 
    colnames(base_data_cumulative_baseline_fit) <- c("cumulative_baseline_haz_values", "time_in_years")
    
    base_cumulative_baseline_haz_fit <- smooth.spline(base_data_cumulative_baseline_fit$time_in_years, base_data_cumulative_baseline_fit$cumulative_baseline_haz_values, df = 25) # similar as above
    
    base_cumulative_baseline_haz_vec_general_func <- function(t){ # t in years 
      cumulative_baseline_value <- ifelse(t > min(base_data_cumulative_baseline_fit$time_in_years),
                                          predict(base_cumulative_baseline_haz_fit, t)$y,
                                          predict(base_cumulative_baseline_haz_fit, min(base_data_cumulative_baseline_fit$time_in_years))$y / min(base_data_cumulative_baseline_fit$time_in_years) * t) # linear interpolation
      cumulative_baseline_value
    }
    
    base_predict_cumulative_baseline_haz_vec_general_func <- function(t){ # t in years 
      predict(base_cumulative_baseline_haz_fit, t)$y
    }
    
    base_min_time_data_cumulative_baseline_haz_fit <- min(base_data_cumulative_baseline_fit$time_in_years)
    
    # Assume that the distributions of covariates are known in the period 2010 - 2015 and they are the same as in the 2000-2010 period
    # Only difference here now is that we simulate excess times from the results of the estimated piecewise excess model

    sim_obs_func <- function(base_em_excess_model) {
      ...
      beta_vec <- base_em_excess_model$coefficients # simulate excess times based on the estimated coefficients of the piecewise excess hazard baseline model
    ...
    # Use the defined function above to simulate CUSUM charts when in control under the assumption that the estimated parameters and shape from piecewise baseline excess hazard model is true
    
    max_cusum_matrix <- CUSUMrelsurv::sim_predefined_obs_func_cusum_prop_threshold_EM_function_parallel(
      sim_obs_func = sim_obs_func,
      base_em_model = base_em_excess_model,
      cumulative_baseline_haz_fit = base_cumulative_baseline_haz_fit,
      baseline_haz_fit = base_baseline_haz_fit,
      min_cumulative_baseline_event_time = base_min_time_data_cumulative_baseline_haz_fit,
      n_iterations = 1000,
      n_cores = 8, 
      rho_vec = c(0.9),
    ...
    store_matrix <- foreach::foreach(
      ...
      vec_time_excess_sim <- CUSUMrelsurv::vec_excess_sim_piecewise(
        partition_t_vec = follow_up_interval_partition,
        baseline_vec = base_baseline_excess_vec,
        max_follow_up_vec = max_follow_up_vec,
        u_vec = u_excess_vec,
        linear_predictor_vec = x_matrix %*% base_beta_vec
      )
      ...
      for (j in 1:n_elements_rho_vec) {
        cusum_chart <- CUSUMrelsurv::cusum_prop_r_t_EM_smoothing_spline(
          baseline_haz_vec_general_func = base_baseline_haz_vec_general_func,
          cumulative_baseline_haz_vec_general_func = base_cumulative_baseline_haz_vec_general_func,
          start_year = start_year,
          age_vec = age_sim_vec,
          gender_vec = gender_sim_vec,
          x_matrix = x_matrix,
...
list_piecewise_in_control_em_predefined <- list(threshold_values = store_threshold, proportion_in_control_signal = proportion_signals_in_control_estimated_vs_true)
saveRDS(list_piecewise_in_control_em_predefined, "section_3_5/piecewise_constant_baseline/semiparametric_model/list_piecewise_in_control_em_predefined_covariates.RDS")

# If the simulations have been performed

list_piecewise_in_control_em_predefined <- readRDS("section_3_5/piecewise_constant_baseline/semiparametric_model/list_piecewise_in_control_em_predefined_covariates.RDS")

df_piecewise_in_control_em_predefined <- data.frame(proportion_in_control_signal = list_piecewise_in_control_em_predefined$proportion_in_control_signal)

hist(df_piecewise_in_control_em_predefined$proportion_in_control_signal, breaks = seq(from = 0, to = 0.55, by = 0.01), probability = T)

ggplot(df_piecewise_in_control_em_predefined, aes(x = proportion_in_control_signal, y = after_stat(density))) + 
  ...
  labs(
    title = "Histogram of signal ratio under in-control (EM and true covariate distributions)",
    x = "Signal ratio",
    y = "Density"
  ) + 
  theme(
