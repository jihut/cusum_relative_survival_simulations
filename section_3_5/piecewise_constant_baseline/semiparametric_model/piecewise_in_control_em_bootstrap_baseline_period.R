...
system.time(
  for (i in 1:n_sims) {
    ...
    max_cusum_matrix <- CUSUMrelsurv::sim_bootstrap_cusum_prop_threshold_EM_function_parallel(
...
list_piecewise_in_control_em_bootstrap <- list(threshold_values = store_threshold, proportion_in_control_signal = proportion_signals_in_control_estimated_vs_true)
saveRDS(list_piecewise_in_control_em_bootstrap, "section_3_5/piecewise_constant_baseline/semiparametric_model/list_piecewise_in_control_em_bootstrap_baseline_period.RDS")

# If the simulations have been performed

list_piecewise_in_control_em_bootstrap <- readRDS("section_3_5/piecewise_constant_baseline/semiparametric_model/list_piecewise_in_control_em_bootstrap_baseline_period.RDS")

df_piecewise_in_control_em_bootstrap <- data.frame(proportion_in_control_signal = list_piecewise_in_control_em_bootstrap$proportion_in_control_signal)

hist(df_piecewise_in_control_em_bootstrap$proportion_in_control_signal, breaks = seq(from = 0, to = 0.55, by = 0.01), probability = T)

ggplot(df_piecewise_in_control_em_bootstrap, aes(x = proportion_in_control_signal, y = after_stat(density))) + 
  ...
  labs(
    title = "Histogram of signal ratio under in-control (EM and bootstrap)",
    x = "Signal ratio",
    y = "Density"
  ) + 
  theme(
