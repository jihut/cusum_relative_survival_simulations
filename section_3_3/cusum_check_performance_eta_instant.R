...
system.time(
  ...
  max_cusum_matrix <- foreach::foreach(
    ...
    for (j in 1:num_of_gamma_values) {
      ...
      vec_time_excess_sim <- CUSUMrelsurv::vec_excess_sim_add_excess_non_neg_piecewise_out(
...
saveRDS(max_cusum_matrix, file = "section_3_3/sim_cusum_check_performance_eta_instant.RDS")

# If the simulations has been performed

max_cusum_matrix <- readRDS("section_3_3/sim_cusum_check_performance_eta_instant.RDS")
...
saveRDS(cusum_signal_df, "section_3_3/cusum_signal_df_eta_instant.RDS")
