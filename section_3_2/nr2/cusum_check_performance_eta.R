...
system.time(
  ...
  max_cusum_matrix <- foreach::foreach(
    ...
    # First 2.5 year - in control (from 2010 - 2012.5), from 2012.5 and beyond we have out of control
...
saveRDS(max_cusum_matrix, file = "section_3_2/nr2/sim_cusum_check_performance_eta.RDS")

# If the simulations has been performed

max_cusum_matrix <- readRDS("section_3_2/nr2/sim_cusum_check_performance_eta.RDS")
...
cusum_signal_df <- data.frame(
  eta = rep("eta = 2.5", nrow(cusum_check_combinations)),
...
saveRDS(cusum_signal_df, "section_3_2/nr2/cusum_signal_df_eta_normal.RDS")
