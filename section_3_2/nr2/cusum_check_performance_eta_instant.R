...
n_iterations <- 1000
rho_vec <- c(0.9, 0.95, 1.05, 1.1) # test out for different values of rho
...
system.time(
  ...
  max_cusum_matrix <- foreach::foreach(
    ...
    for (j in 1:num_of_rho_values) {
      ...
      vec_time_excess_sim <- CUSUMrelsurv::vec_excess_sim_prop_piecewise_out(
...
saveRDS(max_cusum_matrix, file = "section_3_2/nr2/sim_cusum_check_performance_eta_instant.RDS")

# If the simulations has been performed

max_cusum_matrix <- readRDS("section_3_2/nr2/sim_cusum_check_performance_eta_instant.RDS")

max_cusum_matrix_threshold <- readRDS("section_3_2/nr2/sim_cusum_in_control_samples_for_threshold.RDS")
...
saveRDS(cusum_signal_df, "section_3_2/nr2/cusum_signal_df_eta_instant.RDS")
